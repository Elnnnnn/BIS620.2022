...
#' @description The function is to select cilinical trials that have a keyword
#' in a certain column
...
#' @description The function is to plot a histogram of phases of selected table
#'  with a fixed x-axis.
...
plot_phase_histogram <- function(x) {
  ...
  x <- x |>
    select(phase)
  #Bind all phases to the selected table to make sure every phase will appear
  #at least once.
...
#' @description The function is to plot a histogram of conditions of selected
#' studies. The number of conditions is the smaller one of 50 and number of
#' conditions with percentage>0.0001.
#' @param x the database table.
...
get_concurrent_trials <- function(d) {
  ...
  all_dates <- data.frame(date = unique_dates,
                          count = integer(length(unique_dates)))
...
#' @description The function is to get the keywords frequency of
#' selected studies.
#' @param x the studies to get the number of keywords for.
#' @return a matrix containing words existed in the keyword column of x
#' and their frequency.
#' @export
#' @importFrom memoise memoise
#' @importFrom tm tm_map TermDocumentMatrix Corpus content_transformer
#' VectorSource stopwords
#' @importFrom dplyr select collect
get_term_matrix <- memoise(function(x) {
  x <- x |> collect() |> select(keyword) |> as.character()
  my_corpus <- Corpus(VectorSource(x))
  my_corpus <- tm_map(my_corpus, content_transformer(tolower))
  my_corpus <- tm_map(my_corpus, removePunctuation)
  my_corpus <- tm_map(my_corpus, removeNumbers)
  my_corpus <- tm_map(my_corpus, removeWords,
                      c(stopwords("SMART"), "thy", "thou", "thee", "the", "and",
                        "but", "study", "patients"))
...
#' @description The function is to get the number of studies in
#' different countries
...
#' @return table containing nct_id(with link),brief title, start date, and
#' completion date.
#' @export
...
#' @importFrom ggplot2 map_data ggplot geom_polygon coord_fixed
#' scale_fill_gradient theme_minimal labs
plot_states_us <- function(x) {
  ...
  map_data <- merge(states_map, x, by.x = "region", by.y = "state",
                    all.x = TRUE)
...
pie_chart_mesh <- function(x) {
  ...
  # Ordering the data by the 'n' column in descending order and getting the
  #top 10
  plot1 <- head(x[order(-x$n), ], 10)
  # Define colors
  colors <- rainbow(length(plot1$n))
  # Creating the pie chart
  pie(plot1$n, labels = plot1$downcase_mesh_term,
  ...
  legend("topright", legend = pie_labels, fill = colors, bty = "n",
         title = "Percentages")
}
